...
public:
    ...
    string printString(word base = 10, const char *alphabet = "0123456789abcdefghijklmnopqrstuvwxyz")
        const
    {
        vector<char> tablicaCharLiczby;
        if (size() == 0)
        {
            tablicaCharLiczby.push_back('0');
        }
        else
        {
            Num tmp(*this);
            while (tmp.size() > 0)
            {
                word remainder;
                div_mod_half_word(tmp, base, tmp, remainder);
                tablicaCharLiczby.push_back(alphabet[remainder]);
            }
            if (neg)
                tablicaCharLiczby.push_back('-');
            std::reverse(tablicaCharLiczby.begin(), tablicaCharLiczby.end());
        }
        tablicaCharLiczby.push_back('\0');
        string liczbaWynikowa;
        for (int i = 0; i < tablicaCharLiczby.size(); i++)
        {
            liczbaWynikowa += tablicaCharLiczby[i];
        }
...
ostream &operator<<(ostream &os, const Num &liczba)
{
    string wynik = liczba.printString();
    os << wynik;
    return os;
}
